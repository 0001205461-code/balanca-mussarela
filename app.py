...
section[data-testid="stSidebar"] .stButton>button{
    ...
    background:linear-gradient(135deg,rgba(16,22,37,.98),rgba(10,14,25,.98))!important;
...
section[data-testid="stSidebar"] .stButton>button[kind="primary"]{
    background:linear-gradient(90deg,rgba(21,116,255,.98),rgba(112,49,235,.98))!important;
...
/* =====================================================
   ALINHAMENTO DOS FILTROS E BOTÕES
   ===================================================== */
...
/* =====================================================
   TABELAS AMIRA
   Substitui a aparência branca do dataframe nativo.
   ===================================================== */
...
/* Remove espaços excessivos dos containers */
div[data-testid="stVerticalBlock"] > div:has(.amira-table-wrap){
    gap:0!important;
}
</style>
""",
...
def formatar_hora_planilha(valor):
    ...
    if hora:
        ...
        return f"{horas:02d}:{minutos}:{segundos}"
...
def normalizar_lote(valor):
    """
    Mantém lotes iguais juntos mesmo quando a planilha
    vier com pequenas diferenças de formatação.
...
def carregar_dados_todas_abas():
    ...
    for nome_aba, conteudo in dados.items():
        ...
        resultado[str(nome_aba)] = normalizar_colunas(
            pd.DataFrame(
                linhas,
                columns=cabecalho
            )
...
def tabela_amira(df, numero=True, max_height=520):
    """
    Renderiza a tabela com HTML/CSS próprio da AMIRA.
    Assim ela não muda para branco quando o tema externo
    do navegador/Google estiver claro.
    """
...
def agrupar_por_lote(df):
    """
    Junta todas as pesagens que possuem o mesmo lote.

    Exemplo:
    Lote 1524
    19,4 + 19,7 + 18,9 + ...
    =
    Peso total do lote 1524.
    """
    ...
    trabalho["Lote"] = trabalho["Lote"].map(
        normalizar_lote
    )
    ...
    agrupado["_ordem"] = agrupado["Lote"].map(
        ordem_lote
    )
...
def mostrar_producao_por_lote(df):
    ...
    st.markdown(
        ...
        <div class="panel lote-panel">
            ...
            <div class="panel-sub"
                 style="font-size:.80rem;margin-bottom:14px;">
    ...
    tabela["Peso total (kg)"] = tabela[
        "Peso total (kg)"
    ].round(2)

    tabela["Média (kg)"] = tabela[
        "Média (kg)"
    ].round(2)

    tabela["Pesagens"] = tabela[
        "Pesagens"
    ].astype(int)

    tabela_exibicao = tabela.copy()

    tabela_exibicao["Peso total (kg)"] = (
        tabela_exibicao["Peso total (kg)"]
        .map(formatar_numero)
    )

    tabela_exibicao["Média (kg)"] = (
        tabela_exibicao["Média (kg)"]
        .map(formatar_numero)
    )
...
dia_atual = st.session_state.get(
    "dia_selecionado",
    lista_abas[0] if lista_abas else None
)
...
# Agora tudo fica alinhado na mesma linha.
# =========================================================
...
with col_lote:
    ...
    if (
        ...
        lote_selecionado_novo = st.selectbox(
            "🔎 Filtrar por lote",
            options=opcoes_lote,
        ...
        lote_atual = lote_selecionado_novo
...
if menu == "📋  Registro e Dados":
    ...
    mostrar_producao_por_lote(df_filtrado)
    ...
    peso_total, media, total_caixas = resumo_dia(
        df_filtrado
    )
        ...
        for nome in sorted(
            ...
            with st.expander(
                ...
                if temp.empty:
                    ...
                    # Resumo dos lotes da planilha histórica
                    agrupado_historico = agrupar_por_lote(
                        temp
                    )
